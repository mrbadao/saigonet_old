...
function send_mail_order()
	{
		...
		foreach ($cart as $product){
			...
			$dathang.="Ma san pham : ".$pro['code']."<br>"; $order_codelist.= $pro['code']."|";
			$dathang.="Ten san pham : ".$pro['name']."<br>"; 
			$dathang.="So luong : ".$product[1]."<br>";  $order_amoutlist.= $product[1]."|";
			$dathang.="Don gia : ".number_format($pro['price'],0,',','.')."&nbsp;". $currencyUnit."<br>";
			$dathang.="Thanh tien : ".$pro['price']*$product[1]."&nbsp;". $currencyUnit."<br><br>";
			
			$tongcong=$tongcong+$pro['price']*$product[1]."&nbsp;". $currencyUnit;
			$cnt=$cnt+1;
		} 
        
        $order_codelist = substr($order_codelist,0,-1);
        $order_amoutlist = substr($order_amoutlist,0,-1);
        
		$dathang.="<hr>Tong cong : ".number_format($tongcong,0,',','.')."&nbsp;".$currencyUnit."$<br>";
        
        $sql="INSERT INTO `tbl_order` (`id`, `order_name`, `order_company`, `order_address`, `order_city`, `order_country`, `order_phone`, `order_email`, `order_codelist`, `order_amoutlist`, `date_added`, `last_modified`) 
             VALUES (NULL, '".$name."', '".$company."', '".$address."', '".$city."', '".$country."', '".$tel."', '".$email."', '".$order_codelist."', '".$order_amoutlist."', now(), '0000-00-00 00:00:00');";
        
        $result = mysql_query($sql,$conn);
        if($result)
        {	
			return "";
        } else 	return "Không thể gửi thông tin đơn hàng !.";
	}
